<script lang="ts">
  ...
  function generate_code_from_name(name_value: string): string {
    return name_value
      .toUpperCase()
      .replace(/[^A-Z0-9]/g, "_")
      .replace(/_+/g, "_")
  ...
  function handle_name_change(): void {
    if (!code || code === generate_code_from_name(name.slice(0, -1))) {
  ...
  async function handle_submit(event: Event): Promise<void> {
    ...
    show_toast("Role created successfully", "success");
...
<svelte:head>
  <title>Create Staff Role - Sports Management</title>
</svelte:head>
