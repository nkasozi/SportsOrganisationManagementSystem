<script lang="ts">
  ...
  import type { CreateCompetitionFormatInput } from "$lib/domain/entities/CompetitionFormat";
  import {
  ...
  let form_data: CreateCompetitionFormatInput =
    create_empty_competition_format_input();
  ...
  $: requires_group_config = ["groups_knockout", "groups_playoffs"].includes(
    form_data.format_type
  );
  $: requires_knockout_config = [
    "groups_knockout",
    "straight_knockout",
    "double_elimination",
  ].includes(form_data.format_type);
  $: requires_league_config = ["league", "round_robin"].includes(
    form_data.format_type
  );
  ...
  function handle_format_type_change(
    ...
    form_data.format_type = event.detail
      .value as CreateCompetitionFormatInput["format_type"];
  ...
  async function handle_submit(): Promise<void> {
    ...
    if (!result.success) {
      is_submitting = false;
      show_toast(result.error_message || "Failed to create format", "error");
      return;
    }

    show_toast("Competition format created successfully", "success");
  ...
  function handle_status_change(event: CustomEvent<{ value: string }>): void {
    form_data.status = event.detail
      .value as CreateCompetitionFormatInput["status"];
  }
</script>

<svelte:head>
  <title>Create Competition Format - Sports Management</title>
</svelte:head>
  ...
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="text-sm text-accent-600 dark:text-accent-400 mt-1">
        Define a new tournament structure
      </p>
    </div>
  </div>
    ...
    <div class="space-y-4">
      ...
      <div class="flex flex-wrap gap-2">
        {#each TIE_BREAKER_OPTIONS as option}
          <button
            type="button"
            class="px-3 py-2 rounded-lg text-sm font-medium border-2 transition-all {form_data.tie_breakers.includes(
              option.value
            )
          ...
          >
            ...
            {#if form_data.tie_breakers.includes(option.value)}
              <span class="ml-1 text-xs"
                >({form_data.tie_breakers.indexOf(option.value) + 1})</span
              >
            {/if}
          </button>
        {/each}
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <FormField
            ...
            type="number"
            bind:value={form_data.group_stage_config.teams_advancing_per_group}
            min={1}
          />
          ...
          <div class="space-y-3">
            <div class="flex items-center gap-3">
              <input
                ...
                bind:checked={form_data.knockout_stage_config.third_place_match}
            ...
            <div class="flex items-center gap-3">
              <input
                type="checkbox"
                id="two_legged_ties"
                bind:checked={form_data.knockout_stage_config.two_legged_ties}
                class="w-4 h-4 text-primary-600 rounded border-accent-300"
              />
              <label
                for="two_legged_ties"
            ...
            <div class="flex items-center gap-3">
              <input
                type="checkbox"
                id="away_goals_rule"
                bind:checked={form_data.knockout_stage_config.away_goals_rule}
                class="w-4 h-4 text-primary-600 rounded border-accent-300"
              />
              <label
                for="away_goals_rule"
