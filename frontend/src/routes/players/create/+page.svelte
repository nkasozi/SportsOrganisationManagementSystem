<script lang="ts">
  ...
  let form_data: CreatePlayerInput = {
    first_name: "",
    last_name: "",
    date_of_birth: "",
    nationality: "",
  ...
  const preferred_foot_options: Array<{
    value: CreatePlayerInput["preferred_foot"];
    label: string;
  }> = [
  ...
  const status_options: Array<{
    value: CreatePlayerInput["status"];
    label: string;
  }> = [
  ...
  function handle_preferred_foot_change(
    ...
    form_data.preferred_foot = event.detail
      .value as CreatePlayerInput["preferred_foot"];
  }
  ...
  async function handle_submit(): Promise<void> {
    ...
    const result = await player_use_cases.create(form_data);

    if (!result.success) {
      is_submitting = false;
      show_toast(result.error_message || "Failed to create player", "error");
      return;
    }

    show_toast("Player created successfully", "success");
...
<svelte:head>
  <title>Add Player - Sports Management</title>
</svelte:head>
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <EnumSelectField
          ...
          name="preferred_foot"
          value={form_data.preferred_foot}
